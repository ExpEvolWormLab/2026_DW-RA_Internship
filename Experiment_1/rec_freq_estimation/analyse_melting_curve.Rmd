...
```{r}
library(dplyr)
setwd("C:/Users/danme/Desktop/stage/code/rec_freq/test_block1_260506")
...
source(paste0(path,"function_melting_analysis1.R"))
```
...
# red --> high badscore
# blue --> normal
plot.melting.phase(normd = normd, id = "plate1#B6")
...
ggplot()+
  ...
  #theme_Publication2()+
  xlab("EBS-normalized mutant melting signal")+ylab("real rec-1 mutant frequency")+
...
write.table(
  data,
  file = "predicted_frequencies_block1.csv",
  sep = ";",
  row.names = FALSE,
  quote = FALSE
)
...
data_sample <- data %>%
  filter(is.na(calib))
ggplot(data=data_sample,aes(generation, predicted_freq, color= pop_id))+
  facet_wrap(~mix)+
