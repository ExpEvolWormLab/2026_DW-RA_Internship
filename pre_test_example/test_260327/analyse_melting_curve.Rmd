...
design = read_xlsx(paste0("260327_DWRA_design.xlsx"))
```
...
rawd = readHRM(paste0("260327_VDR_test_melting.txt"))
```
...
df <- read.table("260327_VDR_test_ampli.txt", header = TRUE, sep = "\t")
write.table(df, "260327_VDR_test_ampli_2.txt", sep = "\t", dec = ",", row.names = FALSE)
ampli = readAmpli("260327_VDR_test_ampli_2.txt")
...
p <- ggplot() +
  geom_point(
    data = data,
    aes(predicted_freq, ratio,
        color = calib,
        text = well_pcr)  # 👈 关键
  ) +
  xlab("predicted ratio") +
...
```{r}
calib_1 = subset(meltinfo, calib == "calib1") # calibration 
# Train the model on the calibration samples
gam_model_1 <-gam(ratio~s(normEBS.decrease, df=4)  ,data = calib_1)
summary(gam_model_1)

# If you want to plot the model
pgam_1 =data.frame(xx=seq(0.0,1,0.01),
                 yy=predict(gam_model_1, newdata = data.frame(normEBS.decrease=seq(0,1,0.01))) )


# Black line = model
# Dot are calibration samples; colors are independent calibration curves
ggplot()+
  geom_point(data=subset(meltinfo, calib == "calib1"),
             aes(normEBS.decrease, ratio, color=calib), shape=1, size=3,stroke=1)+
  #theme_Publication2()+
  xlab(expression(d[mut]))+ylab("rec-1 mutant frequency")+
  theme(axis.title = element_text(size=17,colour = "black", face = "bold"),    
        ...
        plot.title = element_text(size = 17, hjust = -0.1, face = "bold"),
        legend.position = "none")+
  geom_line(data=pgam_1, aes(x=xx, y=yy),size=1)+
  theme_bw()
...
```
```{r}
calib_2 = subset(meltinfo, calib == "calib2") # calibration 
# Train the model on the calibration samples
gam_model_2 <-gam(ratio~s(normEBS.decrease, df=4)  ,data = calib_2)
summary(gam_model_2)

# If you want to plot the model
pgam_2 =data.frame(xx=seq(0.0,1,0.01),
                 yy=predict(gam_model_2, newdata = data.frame(normEBS.decrease=seq(0,1,0.01))) )


# Black line = model
# Dot are calibration samples; colors are independent calibration curves
ggplot()+
  geom_point(data=subset(meltinfo, calib == "calib2"),
             aes(normEBS.decrease, ratio, color=calib), shape=1, size=3,stroke=1)+
  #theme_Publication2()+
  xlab(expression(d[mut]))+ylab("rec-1 mutant frequency")+
  theme(axis.title = element_text(size=17,colour = "black", face = "bold"),    
        ...
        plot.title = element_text(size = 17, hjust = -0.1, face = "bold"),
        legend.position = "none")+
  geom_line(data=pgam_2, aes(x=xx, y=yy),size=1)+
  theme_bw()
```
